            ...
            while not finished_adding_current_key:
                if key == n.prefix:
                    ...
                    if i == len(key):
                        ...
                        m = SN_RadixNode(n.prefix[:i], n.count + 1, True, {n.prefix[i]:n})
                        n.prefix = n.prefix[i:]
                        ...
                        if key[i] in n.children:
                            ...
                            m = SN_RadixNode(key[i:], 1, True)
                            n.children[key[i]] = m
                            finished_adding_current_key = True
                    else:
                        # The first difference was found before 
                        # the end of key or n.prefix
                        m = SN_RadixNode(n.prefix[:i], n.count + 1, False)
                        m.children[n.prefix[i]] = n
                        m.children[key[i]] = SN_RadixNode(key[i:], 1, True)
                        self._replace_child(n_parent, m)
                        n.prefix = n.prefix[i:]
                        finished_adding_current_key = True
